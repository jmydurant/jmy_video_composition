...
template <typename T>
bool PoissonBlender::solve(const Eigen::SparseMatrix<T, Eigen::ColMajor> &A, const Eigen::Matrix<T, Eigen::Dynamic, 1> &b,
	Eigen::Matrix<T, Eigen::Dynamic, 1> &u)
{
	//Eigen::SparseLU<Eigen::SparseMatrix<T, Eigen::ColMajor>, COLAMDOrdering<int> > lu_of_A;

	//lu_of_A.analyzePattern(A);
	//lu_of_A.factorize(A);

	Eigen::UmfPackLU<const Eigen::SparseMatrix<T, Eigen::ColMajor>> lu_of_A;
	lu_of_A.compute(A);
	...
	for (int y = 1; y < h - 1; ++y) {
		...
		for (int x = 1; x < w - 1; ++x, ++p, ++drv) {
			...
			if (mask1.at<uchar>(y - 1, x) == 0) {
				*drv -= target1.at<cv::Vec3f>(y - 1, x);
				tlrb &= 7; //0b0111
			}
			if (mask1.at<uchar>(y, x - 1) == 0) {
				*drv -= target1.at<cv::Vec3f>(y, x - 1);
				tlrb &= 11; //0b1011
			}
			if (mask1.at<uchar>(y, x + 1) == 0) {
				*drv -= target1.at<cv::Vec3f>(y, x + 1);
...
cv::Mat PoissonBlender::motionAware(cv::Mat & _dst ,cv::Mat & myDiv) {
	ch = _target.channels();
	cv::Point offset(0, 0);
	cv::Point tl(_mask.size()), br(-1, -1);
	...
	cv::Mat hehe;
	myDiv.convertTo(hehe, CV_64F);
	drvxy = Mat(hehe, mask_roi2);
	...
	for (int y = 1; y < h - 1; ++y) {
		...
		for (int x = 1; x < w - 1; ++x, ++pm) {
			if (*pm == 0) {
				...
				int idx = mp[y*(w*ch) + (x*ch)];
				*pd++ = cv::saturate_cast<float>(u[idx + 0]);
				*pd++ = cv::saturate_cast<float>(u[idx + 1]);
				*pd++ = cv::saturate_cast<float>(u[idx + 2]);
			}
		}
	}
	cv::imshow("dst1", myCal::newLUV2RGB(dst1));
	waitKey();
	return dst1;
}